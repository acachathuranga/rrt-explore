...
uint ObstacleFree(std::vector<float> xnear, std::vector<float> &xnew, nav_msgs::msg::OccupancyGrid mapsub){
  ...
  for (int c=0;c<stepz;c++){
    ...
    if (gridValue(mapsub, xi) == 100){return MapGridCellType::OBSTACLE; }
    if (gridValue(mapsub, xi) == -1) { xnew=xi; return MapGridCellType::FRONTIER;}
